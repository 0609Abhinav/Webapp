...
import { CRow, CCol, CCard, CCardHeader, CCardBody, CListGroup, CListGroupItem, CAvatar, CFormInput, CButton, CBadge } from "@coreui/react";
import CIcon from "@coreui/icons-react";
import { cilChatBubble, cilMoon, cilSun } from "@coreui/icons";
  ...
  useEffect(() => {
    ...
    socket.current.on("receive-message", (msg) => {
      if (
        ...
        // Update notification badge for unread messages
        setUnreadCounts((prev) => ({
          ...prev,
          [msg.from]: (prev[msg.from] || 0) + 1,
        }));
                ...
                {filesToUpload.length > 0 && (
                  <div className="mb-2 d-flex flex-wrap">
                    {filesToUpload.map((file, idx) => (
                      <div
                        key={idx}
                        style={{ position: "relative", marginRight: "6px", marginBottom: "6px" }}
                      >
                        {file.type.startsWith("image") ? (
                          <img
                            src={URL.createObjectURL(file)}
